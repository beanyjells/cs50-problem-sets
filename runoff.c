...
int find_min(void)
{
    ...
    for (int z = 1; z < candidate_count; z++)
    {
        ...
        && (candidates[z].votes < least_votes)) // Check that the candidate has not been eliminated and that the votes are less than the variable
        {
...
void eliminate(int min)
{
    ...
    for (int e = 0; e < candidate_count; e++)
    {
        ...
        && (candidates[e].votes == min))// Check that the candidate has not already been eliminated and has the minimum number of votes
        {
